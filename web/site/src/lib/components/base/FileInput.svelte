...
<div class="h-full relative bg-card w-full border rounded-md flex flex-col">
    <Input
            {accept}
            {capture}
    ...
    <div class="flex flex-col gap-4 px-8 pb-8 overflow-y-auto h-full max-h-[590px] custom-scrollbar">
        {#each images as image, i (image.id)}
            <div class="flex p-2 gap-2 relative min-h-56 border border-primary isolate rounded-lg overflow-hidden" in:fly={{ y: -20, opacity: 0, duration: 300 }} out:scale={{ opacity: 0, start: 0.9, duration: 300 }}>
                ...
                <div class="flex items-center gap-2 h-fit">
                    <Button variant="ghost" size="icon" class="text-foreground rounded-full bg-muted/50 hover:bg-muted h-8 w-8" on:click={() => handleRemoveFile(image.id, i)}>
